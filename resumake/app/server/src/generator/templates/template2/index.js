...
const generator: Template2Generator = {
  profileSection(basics) {
    ...
    const { name, age, gender, email, phone, location = {}, website } = basics
    ...
    const genderLine = gender ? `{\\faChild\\ ${gender}}` : ''
    const ageLine = age ? `{\\faChild\\ ${age}}` : ''
    const addressLine = location.address
      ? `{\\faMapMarker\\ ${location.address}}`
      : ''
    const websiteLine = website ? `{\\faLink\\ ${website}}` : ''
    const info = [genderLine, ageLine, emailLine, phoneLine, addressLine, websiteLine]
      .filter(Boolean)
      .join(' | ')
  ...
  summarySection() {
    return source`
      ...
      \\begin{cventries}
      \\begin{itemize}
        \\item This is just a demo Summary.
      \\end{itemize}
      \\vspace{-5mm}
      \\end{cventries}
    \\vspace{10pt}
...
function template2(values: SanitizedValues) {
  ...
  // Insert "Summary" after "profile"
  let index_of_profile = values.sections.indexOf('profile')
  if (index_of_profile !== -1) {
    values.sections.splice(index_of_profile + 1, 0, 'summary');
  }
    ...
    ${values.sections
      .map(section => {
        switch (section) {
          ...
          case 'summary':
            return generator.summarySection()
